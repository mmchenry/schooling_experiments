...
#     cv2.setMouseCallback(win_name, mouse_callback, param=(points, image, image_copy))

#     cv2.imshow(win_name, image)

#     def display_points(image, points):
...
#             cv2.putText(image, f'{distance:.2f} pixels', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
#         if key == ord('q') or key == 13:  # 'q' or return key to quit
#             break
...
#         display_points(frame, points)

    # Clear the image copy to redraw points
    # image_copy = image.copy()
    # image = image_copy.copy()


# def measure_length(frame):
#     """Select a polygon from an image.
#     Args:
#         frame (np.array): Image to select polygon from.
#     Returns:
...
#     # Instructions
#     print("Click to two points to measure length. Press 'Enter' to finish. Right-click to delete the last point.")
