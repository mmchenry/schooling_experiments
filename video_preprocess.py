...
def run_mean_image(path, sch_num, sch_date, analysis_schedule, max_num_frame_meanimage=200, 
    ...
    # Get schedule data
    if sch_date is not None:
        sch = pd.read_csv(path['sch'] + os.sep + sch_date + '.csv')
    else:
        sch = None
        ...
        if (not os.path.exists(mean_image_path)) or overwrite_existing:
            ...
            if trial_specific_mean is True:
                if sch_num is not None:
                    ...
                    cat_curr = cat[(cat['trial_num'] == row.trial_num)]
            ...
            if sch_curr is not None:
                ...
                mean_image = make_max_mean_image_simple(cat_curr, vid_path, max_num_frame_meanimage, 
                                                 im_mask=im_mask, mask_perim=mask_perim, im_crop=True, 
                                                 recorder=recorder, frame_rate=fps)
...
def make_max_mean_image_simple(cat_curr, vid_path, max_num_frames, im_mask=None, mask_perim=None, im_crop=False, 
                        vid_ext_raw='MOV', recorder='ZCam', frame_rate=120):
    """Create a mean image from a video capture object. Selects frames from movies where the light is set at maximum intensity for the batch.
        args:
            cat_cur: Subset of experiment_log to be included in mean image
    ...
    for idx, vid_file in enumerate(df['vid_files'].values):
        ...
        # Collect frame numbers for snippets at max light intensity, for current video
        frame_nums = np.array([])
        frame_nums = np.append(frame_nums, np.linspace(0, total_frames-1, frames_per_vid)).astype(int)
...
def make_mean_image_simple(vid_path, max_num_frames, im_mask=None, mask_perim=None, im_crop=False, 
                         vid_ext_raw='MOV', recorder='ZCam', frame_rate=120):
    """Create a mean image from a video by sampling frames evenly throughout the video.
    
    Args:
        vid_path: Path to video file
        max_num_frames: Maximum number of frames to include in mean image
        im_mask: Mask image (optional)
        mask_perim: Perimeter of mask (required if im_mask is provided)
        im_crop: Whether to crop the image to the region of interest
        vid_ext_raw: File extension of raw video
        recorder: Type of recorder used ('ZCam' or other)
        frame_rate: Frame rate of the video (used if recorder is 'ZCam')
    
    Returns:
        mean_image: Mean image as uint8 numpy array
    """
    # Check if video file exists
    if not os.path.exists(vid_path):
        raise ValueError('Video file does not exist: ' + vid_path)
    ...
    frame_nums = np.linspace(0, total_frames-1, max_num_frames, dtype=int)
    ...
    for frame_num in frame_nums:
        ...
        # Initialize or add to mean image
